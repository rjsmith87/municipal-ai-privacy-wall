...
def invoke_create_case_flow(case_data):
    ...
    payload = {
        'inputs': [{
            ...
            'subtype': case_data.get('subtype', ''),
            'priority': case_data.get('priority', 'Medium'),
            'queueId': case_data.get('queueId', ''),
            'aiSummary': case_data.get('aiSummary', ''),
            'confidence': int(case_data.get('confidence', 80)),
            'safetyFlag': case_data.get('safetyFlag', 'false').lower() == 'true'
        }]
    }
...
def parse_create_case_block(text):
    """Extract case data from [CREATE_CASE]...[/CREATE_CASE] block."""
    match = re.search(r'\[CREATE_CASE\](.*?)\[/CREATE_CASE\]', text, re.DOTALL)
    if not match:
        return None
...
def chat():
    ...
    try:
        ...
        if image_base64:
            ...
            content_doc_id = upload_image_to_salesforce(redacted_b64)
            if content_doc_id:
        ...
        if response.status_code == 200:
            ...
            # Check if Austin wants to create a case
            austin_reply = result.get('reply', '')
            ...
            if case_data:
                ...
                if case_result.get('success'):
                    clean_reply = austin_reply.split('[CREATE_CASE]')[0].strip()
                    result['reply'] = f"{clean_reply}\n\n✅ Case #{case_result['caseNumber']} has been created. A city worker will address this issue."
                    result['caseNumber'] = case_result['caseNumber']
                    result['caseId'] = case_result['caseId']
                else:
                    clean_reply = austin_reply.split('[CREATE_CASE]')[0].strip()
                    result['reply'] = f"{clean_reply}\n\n⚠️ There was an issue creating your case. Please try again or call 311 directly."
                    result['error'] = case_result.get('message')
